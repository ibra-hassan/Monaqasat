...
class QaTopic extends Model
{
    ...
    {
        return $this->hasMany(QaMessage::class, 'topic_id')->orderBy('created_at', 'desc');
    }
    ...
    {
        return $this->creator_id === Auth::user()->id ? Employee::withTrashed()->find($this->receiver_id)
            : Auth::user();
    }
    ...
    public static function unreadCount()
    {
        $topics      = QaTopic::where(function ($query) {
            $query->where('creator_id', Auth::user()->id)->orWhere('receiver_id', Auth::user()->id);
