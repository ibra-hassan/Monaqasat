...
use App\Http\Requests\Admin\QaTopicCreateRequest;
use App\Http\Requests\Admin\QaTopicReplyRequest;
...
class MessengerController extends Controller
{
    public function index()
    {
        $topics  = QaTopic::where(function ($query) {
            $query->where('creator_id', Auth::id())->orWhere('receiver_id', Auth::id());
    ...
    {
        $users   = Employee::all()->except(Auth::id());
        $unreads = $this->unreadTopics();
    ...
    {
        $topic = QaTopic::create(['subject'     => $request->input('subject'), 'creator_id' => Auth::id(),
                                  'receiver_id' => $request->input('recipient'),]);
        $topic->messages()->create(['sender_id' => Auth::id(), 'content' => $request->input('content'),]);
    ...
    public function showInbox()
    {
        ...
        $topics  = QaTopic::where('receiver_id', Auth::id())->orderBy('created_at', 'DESC')->get();
        $unreads = $this->unreadTopics();
    ...
    public function showOutbox()
    {
        ...
        $topics  = QaTopic::where('creator_id', Auth::id())->orderBy('created_at', 'DESC')->get();
        $unreads = $this->unreadTopics();
