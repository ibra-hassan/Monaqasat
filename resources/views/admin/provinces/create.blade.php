    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.provinces.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="required" for="phone">{{ trans('cruds.province.fields.phone') }}</label>
                    <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone"
                           id="phone" value="{{ old('phone', '') }}" required>
                    @if($errors->has('phone'))
                        <span class="text-danger">{{ $errors->first('phone') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.province.fields.phone_helper') }}</span>
                </div>
                @include('partials.save_form', ['slot' => 'province'])
            </form>
        </div>
    </div>
