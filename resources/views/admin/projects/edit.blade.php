            ...
            <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('target_number') ? 'is-invalid' : '' }}" type="number"
                           ...
                           value="{{ old('target_number', $project->target_number) }}" step="1" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text"
                           name="description" id="description" value="{{ old('description', $project->description) }}"
                           required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('estimated_year') ? 'is-invalid' : '' }}" type="number"
                           name="estimated_year" id="estimated_year"
                           value="{{ old('estimated_year', $project->estimated_year) }}" step="100">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('cost_primary') ? 'is-invalid' : '' }}" type="number"
                           name="cost_primary" id="cost_primary"
                           value="{{ old('cost_primary', $project->cost_primary) }}" step="100.00" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type_id"
                        ...
                        @foreach($types as $id => $type)
                            <option
                                value="{{ $id }}" {{ (old('type_id') ? old('type_id') : $project->type->id ?? '') == $id ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('type'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('nature') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($natures as $id => $nature)
                            <option
                                value="{{ $id }}" {{ (old('nature_id') ? old('nature_id') : $project->nature->id ?? '') == $id ? 'selected' : '' }}>{{ $nature }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('directorate') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($directorates as $id => $directorate)
                            <option
                                value="{{ $id }}" {{ (old('directorate_id') ? old('directorate_id') : $project->directorate->id ?? '') == $id ? 'selected' : '' }}>{{ $directorate }}</option>
                        @endforeach
                    </select>
