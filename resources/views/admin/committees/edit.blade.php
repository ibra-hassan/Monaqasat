            ...
            <form method="POST" action="{{ route("admin.committees.update", [$committee->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                           id="name" value="{{ old('name', $committee->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($projects as $id => $project)
                            <option
                                value="{{ $id }}" {{ (old('project_id') ? old('project_id'): $committee->project->id) == $id ? 'selected' : '' }}>{{ $project }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('president') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($presidents as $id => $president)
                            <option
                                value="{{ $id }}" {{ (old('president_id') ? old('president_id') : $committee->president->id) == $id ? 'selected' : '' }}>{{ $president }}</option>
                        @endforeach
                    </select>
